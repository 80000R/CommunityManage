...
<?php
  ...
  //生成日程id
  $dnum = md5(time().mt_rand(1,1000000));
  ...
  //插入之前新的日程表
  $records = $d->query('INSERT INTO date VALUES(?,?,?,?,?,?,?,?) ',array($dnum,$cnum,$dname,$dtime,$dplac,$ps,$id,$phone));
  ...
  //筛选可选学生名单
  $records = $d->query('SELECT stuName,stuID FROM comMember WHERE comNumber = ?',array($cnum));
  $rows = 0;
  $temp = array();
  ...
  <div class="row">
    <div class="col-sm-4">
      ...
      <ul class="nav nav-pills flex-column">
        ...
        <li class="nav-item">
          <a class="nav-link active" href="s_date_pub.php">发布日程</a>
        </li>
        <li class="nav-item">
    ...
    <div class="col-sm-8">
      <h2>人员选择</h2>
      <p>请选择需要参加该日程的成员</p>
      <form method="post" action="s_date_sele_deal.php">
        <input type="hidden" name="dnum" value=<?php echo $dnum?>>
          ...
          //var jsonString = eval('('+jsonString1+')');
              ...
              for(i = 0;i<rows;i++){
                ...
                document.write("<input type='checkbox' class='form-check-input' name = renyuan[] value='"+jsonObject[i][1]+"'>"+jsonObject[i][0]+" "+jsonObject[i][1]);
