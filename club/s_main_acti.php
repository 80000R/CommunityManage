<?php
  ...
  $records = $d->query('SELECT Activity.actID,clubinfo.clubName,Activity.actName,Activity.actTime FROM Activity,clubinfo WHERE Activity.comNumber = clubinfo.No');
...
<div class="box">
  <script>
    ...
    for(i = 0;i<rows;i++){
      document.write("<form method='post' action='s_main_acti_detail.php'>");
      document.write("<h4 class='text-light' class='text-center' >"+jsonObject[i][2]+"</h4>");
      document.write("<h4 class='text-light' class='text-center' >");
      document.write("<small>");
      document.write("<span class='text-light' style='white-space:pre;'>"+"    社团："+jsonObject[i][1]+"    "+"</span>");
      document.write("<span class='text-light' style='white-space:pre;'>"+"时间："+jsonObject[i][3]+"    "+"</span>");
      document.write("<input class='text-light' type='hidden' name = 'anum' value="+jsonObject[i][0]+">");
