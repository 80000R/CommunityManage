...
<?php
  ...
  $records = $d->query('SELECT actName,actID,actTime FROM Activity WHERE resStuID = ? AND comNumber = ?',array($id,$cnum));
  ...
  <div class="row">
    <div class="col-sm-4">
      ...
      <ul class="nav nav-pills flex-column">
        ...
        <li class="nav-item">
          <a class="nav-link active" href="s_acti_mana.php">活动管理</a>
        </li>
        <li class="nav-item">
    ...
    //var jsonString = eval('('+jsonString1+')');
        ...
        for(i = 0;i<rows;i++){
          ...
          document.write("<p>"+"时间："+jsonObject[i][2]+"</p>");
          document.write("<div class='btn-group'>");
          document.write("<form method='post' action='s_acti_modi.php'>");
          document.write("<input type='hidden' name='anum' value="+jsonObject[i][1]+">");
          document.write("<input type='submit' class='btn btn-primary' value='修改'/>");
          document.write("</form>");
          document.write("<form method='post' action='s_acti_conf.php'>");
          document.write("<input type='hidden' name='anum' value="+jsonObject[i][1]+">");
